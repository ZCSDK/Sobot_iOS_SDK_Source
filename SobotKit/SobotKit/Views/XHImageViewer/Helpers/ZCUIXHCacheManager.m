...
#import "ZCUIXHFileAttribute.h"

@interface ZCUIXHCacheManager () {
...
@implementation ZCUIXHCacheManager

+ (instancetype)shareCacheManager {
    return self.manager;
}

+ (ZCUIXHCacheManager *)cacheManagerWithIdentifier:(NSString *)identifier {
    return [[ZCUIXHCacheManager alloc] initWithIdentifier:identifier];
}
...
+ (ZCUIXHCacheManager *)manager {
    static ZCUIXHCacheManager *sharedInstance = nil;
    static dispatch_once_t  onceToken;
    
    dispatch_once(&onceToken, ^{
        sharedInstance = [[ZCUIXHCacheManager alloc] initWithIdentifier:NSStringFromClass(self)];
    });
...
- (NSString *)_cacheDirectory {
    if(_cacheDirectoryPath == nil) {
        ...
        _cacheDirectoryPath = [paths.lastObject stringByAppendingPathComponent:MD5Hash(self.identifier)];
    ...
    for(int i = 0; i < 16; i ++) {
        for(int j = 0; j < 16; j++) {
            ...
            for(id name in list) {
                ...
                if(attr) {
                    [files addObject:[[ZCUIXHFileAttribute alloc] initWithPath:filePath attributes:attr]];
                }
            }
        }
    }
...
- (void)limitNumberOfCacheFiles:(NSInteger)numberOfCacheFiles {
    ...
    for(NSInteger i = numberOfCacheFiles; i < list.count; ++i) {
        ZCUIXHFileAttribute *file = list[i];
...
- (void)removeCacheForURL:(NSURL *)url {
    if(url.absoluteString.length > 0){
        [self _removeCacheForHash:MD5Hash(url.absoluteString)];
    }
}
...
- (void)storeData:(NSData *)data forURL:(NSURL *)url storeMemoryCache:(BOOL)storeMemoryCache {
    if(data && url.absoluteString.length > 0) {
        [self _storeData:data forHash:MD5Hash(url.absoluteString) storeMemoryCache:storeMemoryCache];
    }
}
...
- (NSData *)localCachedDataWithURL:(NSURL *)url {
    if(url.absoluteString.length > 0){
        return [self _localCachedDataWithHash:MD5Hash(url.absoluteString)];
    }
    return nil;
}
...
- (NSData *)dataWithURL:(NSURL *)url storeMemoryCache:(BOOL)storeMemoryCache {
    ...
    return [self _cachedDataWithHash:MD5Hash(url.absoluteString) storeMemoryCache:storeMemoryCache];
}

- (BOOL)existsDataForURL:(NSURL *)url {
    if(url.absoluteString.length > 0) {
        NSString *path = [self _pathForHash:MD5Hash(url.absoluteString)];
...
- (void)storeMemoryCacheWithImage:(UIImage *)image forURL:(NSURL *)url {
    if(image && url.absoluteString.length > 0) {
        [self storeMemoryCacheWithImage:image forHash:MD5Hash(url.absoluteString)];
    }
}
...
- (UIImage *)imageWithURL:(NSURL *)url storeMemoryCache:(BOOL)storeMemoryCache {
    ...
    if(data) {
        ...
        if([data isKindOfClass:[NSData class]]) {
            image = [ZCUIImageTools zc_imageWithData:data];//[ZCUIImageTools fastImageWithData:data];
