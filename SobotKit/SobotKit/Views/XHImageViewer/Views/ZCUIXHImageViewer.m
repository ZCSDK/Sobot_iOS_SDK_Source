...
#import "ZCUIXHViewState.h"
#import "ZCUIXHZoomingImageView.h"
...
@interface ZCUIXHImageViewer () <UIScrollViewDelegate>
...
@implementation ZCUIXHImageViewer
...
- (void)setImageViewsFromArray:(NSArray *)views {
    ...
    for (id obj in views) {
        if ([obj isKindOfClass:[UIImageView class]]) {
            ...
            ZCUIXHViewState *state = [ZCUIXHViewState viewStateForView:view];
...
//                         window.rootViewController.view.transform = CGAffineTransformMakeScale(self.backgroundScale, self.backgroundScale);
                     ...
                     completion:^(BOOL finished) {
                         ...
                         for (ZCUIImageView *view in _imgViews) {
                             view.transform = CGAffineTransformIdentity;
                             
                             if ([view respondsToSelector:@selector(largePhotoURLString)]) {
                                 UIImageView <ZCUIXHImageURLDataSource> *dataSource = (UIImageView <ZCUIXHImageURLDataSource> *)view;
                             ...
                             ZCUIXHZoomingImageView *tmp = [[ZCUIXHZoomingImageView alloc]
...
- (void)dismissWithAnimate:(CGFloat) animate{
    ...
    if(animate == 0){
        ...
        ZCUIXHViewState *state = [ZCUIXHViewState viewStateForView:currentView];
        currentView.frame =
        ...
        for (UIView *view in _imgViews) {
         if ([view respondsToSelector:@selector(largePhotoURLString)]) {
             UIImageView <ZCUIXHImageURLDataSource> *dataSource = (UIImageView <ZCUIXHImageURLDataSource> *)view;
             [((ZCUIImageView *)view) loadWithURL:[NSURL URLWithString:[dataSource thnumbnailPhotoURLString]]];
         }
         ZCUIXHViewState *_state = [ZCUIXHViewState viewStateForView:view];
...
- (void)handlePanGestureRecognizer:(UIPanGestureRecognizer *)sender {
    ...
    if (sender.state == UIGestureRecognizerStateBegan) {
        ...
        while (![targetView isKindOfClass:[ZCUIXHZoomingImageView class]]) {
            targetView = targetView.superview;
        }
        
        if (((ZCUIXHZoomingImageView *)targetView).isViewing) {
            currentView = nil;
        } else {
            UIWindow *window = [[ZCToolsCore getToolsCore] getCurWindow];
            currentView.frame =
