...
REGULAREXPRESSION(URLRegularExpression,(([ZCUICore getUICore].kitInfo!=nil && [ZCUICore getUICore].kitInfo.urlRegular!=nil && [ZCUICore getUICore].kitInfo.urlRegular.length>0) ? [ZCUICore getUICore].kitInfo.urlRegular:@"((http[s]{0,1}|ftp)://[a-zA-Z0-9\\.\\-]+\\.([a-zA-Z0-9]{1,4})(:\\d+)?(/[a-zA-Z0-9\\.\\-~!@#$%^&*+?:_/=<>]*)?)|(([a-zA-Z0-9]{2,4}).[a-zA-Z0-9\\.\\-]+\\.([a-zA-Z]{2,4})(:\\d+)?(/[a-zA-Z0-9\\.\\-~!@#$%^&*+?:_/=<>]*)?)"))

// \\d{3}-\\d{8}|\\d{3}-\\d{7}|\\d{4}-\\d{8}|\\d{4}-\\d{7}|1+[3578]+\\d{9}|400\\d{7}|\\d{8}|\\d{7}
//\\d{3}-\\d{8}|\\d{3}-\\d{7}|\\d{4}-\\d{8}|\\d{4}-\\d{7}|1+[34578]+\\d{9}|\+\\d{2}1+[34578]+\\d{9}|400\\d{7}|400-\\d{3}-\\d{4}|\\d{8}|\\d{7}
REGULAREXPRESSION(PhoneNumerRegularExpression,(([ZCUICore getUICore].kitInfo!=nil && [ZCUICore getUICore].kitInfo.telRegular!=nil && [ZCUICore getUICore].kitInfo.telRegular.length>0) ? [ZCUICore getUICore].kitInfo.telRegular: @"0+\\d{2}-\\d{8}|0+\\d{2}-\\d{7}|0+\\d{3}-\\d{8}|0+\\d{3}-\\d{7}|1+[34578]+\\d{9}|\\+\\d{2}1+[34578]+\\d{9}|400\\d{7}|400-\\d{3}-\\d{4}|\\d{11}|\\d{10}|\\d{8}|\\d{7}"))
...
NSString * const zckURLActions[] = {@"email->",@"url->",@"phoneNumber->",@"at->",@"poundSign->"};
...
//            }
            emojiDictionary = [[ZCUICore getUICore] allExpressionDict];
//        }
...
- (void)commonInit {
    ...
    self.font = [ZCUITools zcgetKitChatFont];
...
//    UIColor *commonLinkColor = [UIColor colorWithRed:0.112 green:0.000 blue:0.791 alpha:1.000];
    UIColor *commonLinkColor = RGBCOLOR(28.5,0,201.75);
    
    //点击时候的背景色
    [mutableActiveLinkAttributes setValue:(__bridge id)[ZCColorWithWhiteAlpha(0.631,1) CGColor] forKey:(NSString *)kZCTTTBackgroundFillColorAttributeName];
...
- (void)setText:(id)text
{
    ...
    NSRegularExpression * const regexps[] = {kEmailRegularExpression(),kURLRegularExpression(),kPhoneNumerRegularExpression(),kAtRegularExpression(),kPoundSignRegularExpression()};
...
//            range = [replaceText rangeOfString:content];
            ...
            [dict setObject:[NSString stringWithFormat:@"%u",match.range.location - len] forKey:@"realFromIndex"];
