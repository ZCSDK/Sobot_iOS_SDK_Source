...
@interface ZCUICore()<ZCMessageDelegate>{
    ...
    ///////////////////////定时器相关/////////////////////////////////
//    int     userTipTime;        // 用户不说话   // 3.0.4版本不在使用
//    BOOL    isUserTipTime;      // 是否提醒了  // 3.0.4版本不在使用
//    int     adminTipTime;       // 客服超时    // 3.0.4版本不在使用
//    BOOL    isAdminTipTime;     // 是否已经提醒 // 3.0.4版本不在使用
...
//@property(nonatomic,assign) BOOL isHistoryLoading;
...
//        _isHistoryLoading = NO;
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(showSoketConentStatus:)]) {
            [self.delegate showSoketConentStatus:ZC_CONNECT_START];
        }
...
//            isLoadingConfig = NO;
            ...
            if((sobotIsNull(_cids) || _cids.count == 0) && !sobotIsNull(safeCore.delegate)){
                // 获取历史记录
...
//            isLoadingConfig = NO;
        ...
        // 平台版新增**************************
        if(![@"" isEqual:sobotConvertToString([ZCLibClient getZCLibClient].platformUnionCode)] && [ZCLibClient isPlatformUnion]){
...
//    }
    ...
    SLog(@"获取历史记录的接口 在调用 sssssssssssssssssss", nil);
    __weak ZCUICore *  weakSelf = self;
//    _isHistoryLoading = YES;
    [_apiServer getHistoryMessages:_curCid withUid:sobotConvertToString([self getPlatfromInfo].config.uid) start:^{
        // 开始加载历史记录
        if(self.delegate && [self.delegate respondsToSelector:@selector(onPageStatusChanged:message:obj:)]){
...
//            }
        ...
        if(weakSelf.isHadCidLoaded && weakSelf.cids!=nil && weakSelf.cids.count>0){
            ...
            if([weakSelf.curCid isEqual:lastCid]|| [[weakSelf getLibConfig].cid isEqualToString:lastCid]){
...
-(void)doConnectUserService:(ZCLibMessage *)msgModel connectType:(ZCTurnType) type{
                ...
                if ([sobotConvertToString(dict[@"code"]) intValue] == 1 && [sobotConvertToString(dict[@"data"][@"openFlag"]) intValue] == 1) {
                    if (self.delegate && [self.delegate respondsToSelector:@selector(coreOpenNewPageVC:IsExist:isShowToat:tipMsg:Dict:Object:trunType:)]) {
                        ...
                        if(sobotConvertToString(_checkGroupId).length > 0){
                            [self.delegate coreOpenNewPageVC:ZC_AskTabelPage IsExist:LeaveExitTypeISCOLSE isShowToat:NO tipMsg:@"clearskillId" Dict:dict Object:msgModel trunType:type];
                        }else{
                            [self.delegate coreOpenNewPageVC:ZC_AskTabelPage IsExist:LeaveExitTypeISCOLSE isShowToat:NO tipMsg:@"" Dict:dict Object:msgModel trunType:type];
                        }
                    }
                }else{
                    // 去执行转人工的操作
    ...
    if(msgModel!=nil && [msgModel isKindOfClass:[ZCLibMessage class]]){
        /// 3.1.5 新增 answerMsgId ruleId 字段
        paramter.answerMsgId = sobotConvertToString(msgModel.msgId);
        paramter.ruleId = sobotConvertToString(msgModel.richModel.ruleId);
        
        if(type == ZCTurnType_KeyWord){
            paramter.queueFlag = msgModel.queueFlag;
            
            paramter.keyword = sobotConvertToString(msgModel.keyword);
            paramter.keywordId = sobotConvertToString(msgModel.keywordId);
        }
        
        paramter.transferType = msgModel.transferType;
        ...
        paramter.unknownQuestion = sobotConvertToString(msgModel.originQuestion);
    }
    ...
    [self saveGroupIdWith:sobotConvertToString(paramter.groupId)];
    ...
    [_apiServer connectOnlineCustomer:paramter config:[self getLibConfig] start:^{
        ...
        [safeVC configConnectedResult:dict code:status ZCTurnType:type];
        
        // configConnectedResult 函数中已对参数做了设置
...
-(void)checkUserServiceWithType:(ZCTurnType) type model:(ZCLibMessage *) message{
    ...
    if (self.isShowForm && !self.isDoConnectedUser) {
        [self doConnectUserService:message connectType:type];
        return;
    }
    
    // 是否开启智能路由 并且不是再次转人工（已经执行过一次智能路由转人工了）
    if ([self getPlatfromInfo].config.smartRouteInfoFlag == 1 && !self.isDoConnectedUser) {
        [self doConnectUserService:message connectType:type];
        return;
    }
...
//        [ZCLibClient getZCLibClient].libInitInfo.groupid = @"";
        ...
        if ([self getLibConfig].msgFlag == 1 && [self getLibConfig].type == 2) {
            if(_delegate && [_delegate respondsToSelector:@selector(coreOpenNewPageVC:IsExist:isShowToat:tipMsg: Dict:Object:trunType:)]){
                //  跳转到留言不直接退出SDK
                [self.delegate coreOpenNewPageVC:ZC_LeaveMsgPage IsExist:LeaveExitTypeISNOCOLSE isShowToat:YES tipMsg:@"" Dict:nil Object:nil trunType:ZCTurnType_BtnClick];
            }
        }else{
            // 添加暂无客服在线提醒
       ...
       _skillSetView  = [[ZCUISkillSetView alloc] initActionSheet:groupArr  withView:(UIView *)_delegate];
        ...
        [_skillSetView setItemClickBlock:^(ZCLibSkillSet *itemModel) {
            ...
            if(!itemModel.isOnline ){
                ...
                // 加载动画
                [[ZCUIToastTools shareToast] showProgress:@"" with:(UIView *)safeSelf.delegate];
        ...
        [_skillSetView setCloseBlock:^{
            ...
            if([self getPlatfromInfo].config.type == 2){
                // 直接关闭技能组
                if(safeCore.delegate && [_delegate respondsToSelector:@selector(onPageStatusChanged:message:obj:)]){
                    [safeCore.delegate onPageStatusChanged:ZCInitStatusCloseSkillSet message:ZCSTLocalString(@"") obj:nil];
                }
...
-(BOOL)checkAfterConnectUser:(ZCTurnType ) type{
    if([self getLibConfig].invalidSessionFlag == 1 && ([self getLibConfig].type == 2||[self getLibConfig].type == 4) && _afterModel==nil){
        ...
        if([self getLibConfig].type == 4){
            ...
            if ([ZCUICore getUICore].libInfo.good_msg_type >0 && sobotConvertToString([ZCUICore getUICore].libInfo.content).length > 0) {
...
#pragma mark - 人机问答接口返回数据 后 关键字转人工、工单节点、关键字转人工增加提示消息、
...
//        }
    ...
    if(message.transferFlag == 0 || message.keywordId.length == 0){
        // 普通回复消息
        if (!sobotIsNull(message.answersList) && message.answersList.count > 0) {
            for (ZCLibMessage *msg in message.answersList) {
                [_listArray insertObject:msg atIndex:index + 1];
                index = index + 1;
            }
        }else{
        ...
        //        _keyworkRobotReplyModel = model;
            if ([self getLibConfig].type == 1 && message.keywordId.length >0) {
                // 仅机器人模式 并且是关键字转人工  显示 机器人回复内容 同 普通消息
//                [_listArray insertObject:model atIndex:index+1];
//                index = index + 1;
                if (!sobotIsNull(message.answersList) && message.answersList.count > 0) {
                    for (ZCLibMessage *msg in message.answersList) {
...
//        trunModel.showTurnUser = message.showTurnUser;
        ...
        // 将转人工的数据逐一封装到 answerslist 中
        if (!sobotIsNull(message.answersList) && message.answersList.count > 0) {
            for (ZCLibMessage *meg in message.answersList) {
                [meg getNewMessageModel:trunModel isShowGroup:YES];
                meg.commentType = trunModel.commentType;
                meg.satisfactionCommtType = trunModel.satisfactionCommtType;
                meg.ratingCount = trunModel.ratingCount;
                meg.isQuestionFlag = trunModel.isQuestionFlag;
                meg.transferFlag = trunModel.transferFlag;
            }
        }
        ...
        if (message.transferFlag == 2) {
            ...
            if(message.queueFlag==1 || (message.queueFlag==0 && message.onlineFlag==1)){
                if(sobotConvertToString(message.transferTips).length > 0){
                    // 提示语
...
//                    trunModel.tipStyle = 0;//ZCReceivedMessageTransferTips;
                    ...
                    [ZCUITools zcModelStringToAttributeString:trunModel];
                    trunModel.answersList = [NSMutableArray array];
                    // 将转人工的数据逐一封装到 answerslist 中
//                    if (!sobotIsNull(message.answersList) && message.answersList.count > 0) {
//                        for (ZCLibMessage *meg in message.answersList) {
//                            [meg getNewMessageModel:trunModel isShowGroup:YES];
...
//                    }
            ...
            if(message.transferFlag == 1){
                ...
                if(message.queueFlag == 1){
                    if(message.onlineFlag == 1){
                        //关键字转人工 直接转人工
                        _checkGroupId = sobotConvertToString(trunModel.groupId);
                        _checkGroupName = @"";
                    }
                    //关键字转人工 直接转人工
                    [self doConnectUserService:trunModel connectType:ZCTurnType_KeyWordNoGroup];
                }else{
                    if(message.onlineFlag == 1 || message.onlineFlag == 2){
                        ...
                        _checkGroupId = sobotConvertToString(trunModel.groupId);
                        _checkGroupName = @"";
                        [self doConnectUserService:trunModel connectType:ZCTurnType_KeyWord];
    ...
    if ([safeVC getPlatfromInfo].config.type != 1 && ![self getLibConfig].isArtificial  && (message.transferType ==1 || message.transferType == 2 || message.transferType == 3 || message.transferType == 5) && !message.userOffline) {
        ...
        [[ZCUICore getUICore] checkUserServiceWithType:ZCTurnType_RepeatOrMood model:model];
    }
...
//        [ZCUICore getUICore].isShowForm = NO;
        ...
        // 执行转人工的操作，由于缺少message对象已放到调用地方执行
        return;
    }
    ...
    if(status == ZCConnectUserBeBlock){// 说明当前用户是黑名单用户
        ...
        if([self showChatAdminHello]){
            ...
            message.senderFace = sobotConvertToString(dict[@"data"][@"aface"]);
            [message getModelDisplayText:YES];//处理欢迎语中包含富文本标签
            // 发送人工欢迎语，并添加 商品和订单信息
...
//            // 1.客服欢迎语开关没有开启 2.开启了欢迎语 但是是只显示一次的，并且已经是显示过了
//            [self checkAddGoodsAndOrderMessage];
//            // 自动发送商品卡片信息
//            [self autoSendLastMessageToUser];
//        }
        [self addAutoSendAdminMessageWithCheckAddMsg:YES isAddGoodMsg:YES isAutoSendMsg:YES];
        ...
        ZCLibMessage *message = [ZCLibServer setLocalDataToArr:ZCTipMessageWaiting type:ZCReceivedMessageWaiting duration:@"" style:messageType send:NO name:self.receivedName content:sobotConvertToString(dict[@"data"][@"queueDoc"]) config:[self getPlatfromInfo].config];
...
//    }
    ...
    temModel.date         =  sobotGetCurrentTimes();
    ...
    }else if(action == ZCTipMessageUnresolvedProblemTurn){
        msg = [NSString stringWithFormat:@"%@ %@",ZCSTLocalString(@"未解决问题？点击"),ZCSTLocalString(@"转人工服务")];
    }else{
    ...
    temModel.ts           = sobotGetCurrentTimes();//sobotGetCurrentTimes();
    ...
    }else if(tipStyle == ZCTipMessageUnresolvedProblemTurn){
        temModel.tipStyle = tipStyle;
        temModel.sysTips = msg;
    }else{
        // 人工回复时，等于7是富文本
...
//            [self.delegate onPageStatusChanged:ZCShowStatusChangedTitle message:ZCSTLocalString(@"暂无客服在线") obj:nil];
    ...
    if(type==ZCReceivedMessageOnline){
        [self removeListModelWithType:0 tips:ZCTipMessageChat_WaitingContinueTips];
...
//        }
        ...
        if (type == ZCReceivedMessageOfflineByAdmin || type == ZCReceivedMessageOfflineByClose) {
            ...
            if ([self getPlatfromInfo].config.serviceEndPushFlag == 1 && ![[self getPlatfromInfo].config.serviceEndPushMsg isEqualToString:@""]) {
                ...
                pushMsg = [pushMsg stringByReplacingOccurrencesOfString:@"#客服#" withString:_receivedName];
                pushMsg = [pushMsg stringByReplacingOccurrencesOfString:@"#agent#" withString:_receivedName];
    ...
    if (type == ZCReceivedMessageEvaluation){
        ...
        [SobotLog logDebug:@"当前发送状态：%d",isUser];
...
//            [self addReceivedNameMessageToList:message IsAdminHelloWord:YES];
    ...
    if (type == ZCReceivedMessageOfflineToLong
        ...
        && ![[ZCIMChat getZCIMChat] getIsChatPageActive]) {
    ...
    if (message.tipStyle == ZCReceivedMessageWaiting) {
        ...
        if (_listArray !=nil && _listArray.count>0 && !conf.isArtificial) {
            ...
            for (int i = (int)_listArray.count-1; i>=0 ; i--) {
                ZCLibMessage *libMassage = _listArray[i];
                if ( [sobotConvertToString(libMassage.sysTips) hasSuffix:ZCSTLocalString(@"留言")] ) {// 2.6.4 去掉|| [libMassage.sysTips hasPrefix:ZCSTLocalString(@"排队中，您在队伍中")]
    ...
    if (message.tipStyle == ZCReceivedMessageOnline) {
        ...
        if (_listArray !=nil) {
            ...
            for (int i = (int)_listArray.count-1; i>=0; i--) {
                ...
                // 删除上一条留言信息
                if ([sobotConvertToString(libMassage.sysTips) hasSuffix:ZCSTLocalString(@"留言")] || [libMassage.sysTips isEqualToString:conf.adminNonelineTitle]) {
...
#pragma Mark - 发送商品和订单信息 不在参考是否显示人工客服欢迎语
- (void)addAutoSendAdminMessageWithCheckAddMsg:(BOOL)isCheck isAddGoodMsg:(BOOL)isAddGoodMsg isAutoSendMsg:(BOOL)isAutoSendMsg{
    // 是否添加商品信息
    if (isCheck) {
        [self checkAddGoodsAndOrderMessage];
    }
    ...
    [[ZCUICore getUICore] loadSatisfactionDictlock:^(int code) {
        if(code == 0 && [self getLibConfig].isArtificial){
            // 如果是人工，需要刷新一下页面，防止推送的评价，显示标签错误
            if(self.delegate && [self.delegate respondsToSelector:@selector(onPageStatusChanged:message:obj:)]){
                [self.delegate onPageStatusChanged:ZCShowStatusMessageChanged message:@"" obj:_listArray];
            }
        }
...
-(void)initConfigData:(BOOL)isFrist IsNewChat:(BOOL) isNew{
    ...
    if (isNew) {
        ...
        [ZCLibClient getZCLibClient].libInitInfo.groupid = sobotConvertToString([[NSUserDefaults standardUserDefaults] valueForKey:@"UserDefaultGroupID"]);
        [ZCLibClient getZCLibClient].libInitInfo.group_name = sobotConvertToString([[NSUserDefaults standardUserDefaults] valueForKey:@"UserDefaultGroupName"]);
        // 重新设置判定参数
        self.isSendToUser = NO;
...
//        isLoadingConfig = NO;
        ...
        if(sobotConvertToString(errorMessage).length > 0 && self.delegate!=nil){
            ...
            if(safeSelf.ResultBlock){
                safeSelf.ResultBlock(ZCInitStatusFail,safeSelf.listArray,sobotConvertToString(errorMessage).length > 0 ? errorMessage : @"初始化失败");
            }
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
- (void)thankFeedBack:(int)type rating:(float)rating IsResolve:(int)isresolve{
    ...
    temModel.senderFace   = @"";
    temModel.t=[NSString stringWithFormat:@"%ld",(long)[[NSDate date] timeIntervalSince1970]];
    temModel.ts           = sobotGetCurrentTimes();
...
-(void)thankFeedBack{
    ...
    if([self getLibConfig].isArtificial && self.kitInfo.isCloseAfterEvaluation){
        ...
        // 加上这一句，下次进入会立即初始化
        // 处理之前先存一下临时数据uid
//        tempUid = [[ZCPlatformTools sharedInstance] getPlatformInfo].config.uid;
        
        [ZCUICore getUICore].currtChatOver = YES;
        // 这里暂时先不置空 点击留言以及留言后面的数据会使用
        ...
        if (_listArray.count>=1) {
            ...
            if (_listArray !=nil) {
                ...
                for (int i = (int)_listArray.count-1; i>=0; i--) {
                    ...
                    // 删除上一条留言信息
                    if ([model.sysTips hasPrefix:sobotConvertToString([self getPlatfromInfo].config.adminNonelineTitle)] && (action == ZCTipMessageUserNoAdmin)) {
                        indexs = [indexs stringByAppendingFormat:@",%d",i];
...
-(void) sendMessage:(NSString *)text questionId:(NSString*)question type:(ZCMessageType) type duration:(NSString *) time dict:(NSDictionary *)extendParams{
    ...
    if(self.kitInfo.isOpenActiveUser && type == ZCMessageTypeText && ![self getPlatfromInfo].config.isArtificial && [self getPlatfromInfo].config.type!=1){
        if(!sobotIsNull(self.kitInfo.activeKeywords) && [self.kitInfo.activeKeywords objectForKey:text]){
...
//            [self turnUserService:nil object:nil];
            [[ZCUICore getUICore] checkUserServiceWithType:ZCTurnType_CustomKeyWord model:nil];
            return;
        }
    }
...
//    }
    ...
    [_apiServer sendMessage:text questionId:question msgType:(ZCMessageType)type duration:time config:[self getPlatfromInfo].config robotFlag:[NSString stringWithFormat:@"%d",[self getLibConfig].robotFlag] dict:extendParams start:^(ZCLibMessage *message) {
        ...
        if(sendCode==ZC_SENDMessage_New){
            ...
            if(self.delegate && [self.delegate respondsToSelector:@selector(onPageStatusChanged:message:obj:)]){
                [self.delegate onPageStatusChanged:ZCSetKeyBoardStatus message:@"ZCKeyboardStatusFrameChanged" obj:safeVC.listArray];
            }
...
-(void)sendAfterConnectUserMessage{
    ...
    if([self getLibConfig].isArtificial){
        // 如果已经是人工了，发送普通消息
        [self sendMessage:sobotConvertToString(_afterModel.richModel.msg) questionId:@"" type:_afterModel.richModel.msgType duration:sobotConvertToString(_afterModel.richModel.duration)];
...
//    __weak ZCUICore *safeVC = self;
//    __block ZCLibMessage *sendMessage  = nil;
//    [_apiServer sendFirstMsgToUser:sobotConvertToString(_afterModel.richModel.msg) config:[self getPlatfromInfo].config msgType:_afterModel.richModel.msgType start:^(ZCLibMessage *message) {
//
//        message.sendStatus=1;
//        sendMessage = message;
//        [ZCUITools zcModelStringToAttributeString:sendMessage];
...
-(ZCLibMessage *)setLocalDataToArr:(int) action
    ...
    if(user_nick!=nil && ![@"" isEqual:sobotConvertToString(user_nick)]){
...
-(void)autoSendLastMessageToUser{
    ...
    if (_kitInfo.productInfo!=nil && [self getPlatfromInfo].config.isArtificial  && ![@"" isEqualToString:_kitInfo.productInfo.title] && ![@"" isEqualToString:_kitInfo.productInfo.link] && _kitInfo.isSendInfoCard) {
        ...
        [contentDic setObject:[NSString stringWithFormat:@"%@",sobotConvertToString(_kitInfo.productInfo.title)] forKey:@"title"];
        
        [contentDic setObject:[NSString stringWithFormat:@"%@",sobotConvertToString(_kitInfo.productInfo.desc)] forKey:@"description"];
        
        [contentDic setObject:[NSString stringWithFormat:@"%@",sobotConvertToString(_kitInfo.productInfo.label)] forKey:@"label"];
        
        [contentDic setObject:[NSString stringWithFormat:@"%@",sobotConvertToString(_kitInfo.productInfo.link)] forKey:@"url"];
        
        [contentDic setObject:[NSString stringWithFormat:@"%@",sobotConvertToString(_kitInfo.productInfo.thumbUrl)] forKey:@"thumbnail"];
        // 转json
    ...
    if (_kitInfo.orderGoodsInfo!=nil && [self getPlatfromInfo].config.isArtificial && _kitInfo.autoSendOrderMessage) {
        ...
        [contentDic setObject:sobotConvertToString(_kitInfo.orderGoodsInfo.statusCustom) forKey:@"statusCustom"];
        [contentDic setObject:sobotConvertToString(_kitInfo.orderGoodsInfo.createTime) forKey:@"createTime"];
        [contentDic setObject:sobotConvertToString(_kitInfo.orderGoodsInfo.orderCode) forKey:@"orderCode"];
        [contentDic setObject:sobotConvertToString(_kitInfo.orderGoodsInfo.createTime) forKey:@"createTime"];
        [contentDic setObject:_kitInfo.orderGoodsInfo.goods forKey:@"goods"];
        [contentDic setObject:sobotConvertToString(_kitInfo.orderGoodsInfo.orderUrl) forKey:@"orderUrl"];
        [contentDic setObject:sobotConvertToString(_kitInfo.orderGoodsInfo.goodsCount) forKey:@"goodsCount"];
        [contentDic setObject:sobotConvertToString(_kitInfo.orderGoodsInfo.totalFee) forKey:@"totalFee"];
        // 转json
        contextStr = [ZCLocalStore DataTOjsonString:contentDic];
...
-(void)addGoodMsg{
    ...
    // 添加自动发送自定义消息，客户单独要求需要每次都发送
    if ([ZCUICore getUICore].libInfo.good_msg_type >0 && ![sobotConvertToString([ZCUICore getUICore].libInfo.content) isEqualToString:@""]) {
        // 机器人和人工都发送
        if([ZCUICore getUICore].libInfo.good_msg_type > 1 && [self getPlatfromInfo].config.isArtificial){
            ...
            [[ZCUICore getUICore] sendMessage:sobotConvertToString([ZCUICore getUICore].libInfo.content) questionId:@"" type:type duration:@""];
        }
        // 自动给机器人发送文本信息
...
-(void)timerCount{
    ...
    // 用户超时，此处不处理了，改由服务器判断
#pragma Mark 3.0.4开始 客服超时由服务端处理
    // 用户长时间不说话,人工才添加提示语
...
-(NSDictionary *)getZCThemeColorDict:(NSString *)style{
    ...
    return colorThemeDict[sobotConvertToString(style)];
}
...
/**
 *
 *  是否添加转人工提示语    ” 未解决问题？ 点击 转人工服务“
 */
-(void)addTipTurnToArtificialMsg{
    [self addTipsListenerMessage:ZCTipMessageUnresolvedProblemTurn];
}
...
-(void)continueWaiting:(ZCLibMessage *)tipsModel{
    [[ZCLibServer getLibServer] continueWaiting:[self getLibConfig] start:^{
        
    } success:^(NSDictionary *dict, ZCNetWorkCode sendCode) {
        tipsModel.isHistory = YES;
        [[ZCUICore getUICore] removeListModelWithType:0 tips:ZCTipMessageChat_WaitingContinueTips];
        if (self.delegate && [self.delegate respondsToSelector:@selector(onPageStatusChanged:message:obj:)]) {
            [self.delegate onPageStatusChanged:ZCShowStatusMessageChanged message:@"" obj:[ZCUICore getUICore].listArray];
        }
    } failed:^(NSString *errorMessage, ZCNetWorkCode errorCode) {
...
-(void)removeListModelWithType:(ZCMessageType ) type tips:(ZCTipMessageType) tips{
    if(type > 0){
        for (ZCLibMessage *msg in _listArray) {
            if(msg.richModel.msgType == type){
                [_listArray removeObject:msg];
                break;
            }
        }
    }
...
-(BOOL)showChatAdminOutTips{
    ...
    if([self getLibConfig].serviceOutTimeFlag == 1){
        ...
        if([self getLibConfig].serviceOutCountRule == 1){
            ...
            NSString *isSave =  sobotConvertToString([userDefatluts objectForKey:[NSString stringWithFormat:@"sobot_cid_adminouttips%@",[self getLibConfig].cid]]);
            if(isSave.length > 0){
                // 已经显示过了
                return NO;
            }else{
...
-(NSString*)getTempGroupId{
    NSUserDefaults *userDefatluts = [NSUserDefaults standardUserDefaults];
    NSString *groupId = sobotConvertToString([userDefatluts objectForKey:[NSString stringWithFormat:@"sobot_temp_groupid"]]);
    
    if(groupId.length == 0){
        groupId = sobotConvertToString(_libInfo.groupid);
    }
    return groupId;
}
