...
#define myScreenHeight  ([UIScreen mainScreen].bounds.size.height - XBottomBarHeight)
...
-(id)initWithImgULocationArr:(NSMutableArray*)array CurPage:(NSInteger)curpage
{
    ...
    if (self) {
        ...
        if ([ZCUICore getUICore].kitInfo.navcBarHidden) {
            ...
            [self.titleLabel setText:[NSString stringWithFormat:@"%d/%zd",mcurpage+1,array.count]];
            ...
            self.title = [NSString stringWithFormat:@"%d/%zd",mcurpage+1,array.count];
...
-(IBAction)buttonClick:(UIButton *) sender{
    ...
    if(sender.tag == BUTTON_MORE){
        ZCActionSheet *mysheet = [[ZCActionSheet alloc] initWithDelegate:self selectedColor:UIColorFromRGB(TextCleanMessageColor) showTitle:ZCSTLocalString(@"要删除这张图片吗？") CancelTitle:ZCSTLocalString(@"取消") OtherTitles:ZCSTLocalString(@"删除"), nil];
...
-(void)changePageTitle{
    if ([ZCUICore getUICore].kitInfo.navcBarHidden) {
        [self.titleLabel setText:[NSString stringWithFormat:@"%ld/%zd",pageControl.currentPage+1,pageControl.numberOfPages]];
    }else{
        self.title = [NSString stringWithFormat:@"%ld/%zd",pageControl.currentPage+1,pageControl.numberOfPages];
    }
}
